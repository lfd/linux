...
/* MUST be called with lpriv->dirty_lock held */
static void __imsic_local_sync(struct imsic_local_priv *lpriv)
{
	...
	/*
	 * The IPI-mux already has required barriers so no need for
	 * any additional barriers here.
	 */
	...
	for_each_set_bit(i, lpriv->dirty_bitmap, imsic->global.nr_ids + 1) {
		...
		if (atomic_long_read(&lpriv->vectors[i].enable))
			__imsic_id_set_enable(i);
		else
			__imsic_id_clear_enable(i);

		mvec = (struct imsic_vector *)atomic_long_xchg(
						&lpriv->vectors[i].move, 0);
		if (mvec) {
...
void imsic_local_sync_all(void)
{
	...
	raw_spin_lock_irqsave(&lpriv->dirty_lock, flags);
	bitmap_fill(lpriv->dirty_bitmap, imsic->global.nr_ids + 1);
	__imsic_local_sync(lpriv);
	raw_spin_unlock_irqrestore(&lpriv->dirty_lock, flags);
}
...
/* MUST be called with lpriv->dirty_lock held */
#ifdef CONFIG_SMP
static void imsic_local_dirty_timer_callback(struct timer_list *timer)
{
	struct imsic_local_priv *lpriv = this_cpu_ptr(imsic->lpriv);
	unsigned long flags;
...
static void __imsic_remote_sync(struct imsic_local_priv *lpriv,
				unsigned int cpu)
{
	/*
	 * The IPI-mux already has required barriers so no need for
	...
	if (cpu_online(cpu)) {
		if (cpu != smp_processor_id()) {
			if (!timer_pending(&lpriv->dirty_timer)) {
				lpriv->dirty_timer.expires = jiffies + 1;
				add_timer_on(&lpriv->dirty_timer, cpu);
			}
		} else {
			__imsic_local_sync(lpriv);
		}
			...
			struct imsic_vector *new_vec)
{
	...
	/* Mask old vector and unmask new vector */
	atomic_long_set(&new_vec->enable,
			atomic_long_xchg(&old_vec->enable, 0));
	 ...
	 * to the new vector.
	 */
	atomic_long_set(&old_vec->move, (long)new_vec);

	/* Mark the old vector as dirty and synchronize */
	raw_spin_lock_irqsave(&old_lpriv->dirty_lock, flags);
	bitmap_set(old_lpriv->dirty_bitmap, old_vec->local_id, 1);
	__imsic_remote_sync(old_lpriv, old_vec->cpu);
	raw_spin_unlock_irqrestore(&old_lpriv->dirty_lock, flags);

	/* Mark the new vector as dirty and synchronize */
	raw_spin_lock_irqsave(&new_lpriv->dirty_lock, flags);
	bitmap_set(new_lpriv->dirty_bitmap, new_vec->local_id, 1);
	__imsic_remote_sync(new_lpriv, new_vec->cpu);
	raw_spin_unlock_irqrestore(&new_lpriv->dirty_lock, flags);
}
			     ...
			     struct imsic_vector *vec, int ind)
{
	...
	is_enabled = atomic_long_read(&vec->enable) ? true : false;
	mvec = (struct imsic_vector *)atomic_long_read(&vec->move);
	if (mvec) {
...
static int __init imsic_local_init(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		raw_spin_lock_init(&lpriv->dirty_lock);

		/* Allocate dirty bitmap */
		lpriv->dirty_bitmap = bitmap_zalloc(global->nr_ids + 1,
						    GFP_KERNEL);
...
#ifdef CONFIG_SMP
		/* Setup dirty timer */
		timer_setup(&lpriv->dirty_timer,
			    imsic_local_dirty_timer_callback, TIMER_PINNED);
#endif
...
#ifdef CONFIG_SMP
	raw_spin_lock_irqsave(&lpriv->dirty_lock, flags);
	WARN_ON_ONCE(try_to_del_timer_sync(&lpriv->dirty_timer) < 0);
	raw_spin_unlock_irqrestore(&lpriv->dirty_lock, flags);
#endif
}
