...
int aplic_direct_setup(struct device *dev, void __iomem *regs)
{
	...
	rc = aplic_setup_priv(priv, fwnode, regs);
	if (rc) {
		pr_err("%pfwP: failed to create APLIC context\n", fwnode);
		kfree(direct);
		return rc;
	}
	...
	direct->irqdomain = irq_domain_create_linear(fwnode,
						   priv->nr_irqs + 1,
						   &aplic_direct_irqdomain_ops,
						   priv);
	if (!direct->irqdomain) {
		pr_err("%pfwP: failed to create direct irq domain\n", fwnode);
		kfree(direct);
		return -ENOMEM;
	}
	...
	pr_info("%pfwP: %d interrupts directly connected to %d CPUs\n",
		fwnode, priv->nr_irqs, priv->nr_idcs);
