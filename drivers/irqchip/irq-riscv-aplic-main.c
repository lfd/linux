...
int aplic_setup_priv(struct aplic_priv *priv, struct fwnode_handle *fwnode,
		     void __iomem *regs)
{
	struct fwnode_reference_args parent;
	int rc;

	/* Save fwnode and register base */
	priv->fwnode = fwnode;
	priv->regs = regs;
...
static int aplic_probe(struct platform_device *pdev)
{
	 ...
	 */
	if (fwnode_property_present(fwnode, "msi-parent"))
		rc = aplic_msi_setup(&pdev->dev, regs);
	else
		rc = aplic_direct_setup(&pdev->dev, regs);
	if (rc) {
		pr_err("%pfwP: failed setup APLIC in %s mode\n", fwnode,
			fwnode_property_present(fwnode, "msi-parent") ?
			"MSI" : "direct");
		return rc;
	}
